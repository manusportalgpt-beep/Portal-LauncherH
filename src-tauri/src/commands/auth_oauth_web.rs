            ...
            for stream in listener.incoming().take(1) {
                if let Ok(mut stream) = stream {
                    ...
                    if let Ok(n) = stream.read(&mut buffer) {
                        ...
                        // Парсим код из запроса
                        if let Some((code, state)) = parse_oauth_callback(&request) {
                            log::info!("✅ Extracted code and state");
                            ...
                            if let Some(verifier) = code_verifier {
                                // Эмитим событие с кодом
                                let _ = app.emit("oauth-code-received", serde_json::json!({
                                    "code": code,
                                    "code_verifier": verifier,
                                    "state": state,
                                }));
                                log::info!("✅ Emitted oauth-code-received event");
                            } else {
                                log::warn!("⚠️ No code_verifier found in state");
                            }
                        }
                    }
                    ...
                    let response = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nCache-Control: no-cache\r\n\r\n\
                        ...
                        <p>You can now close this window and return to Portal Launcher.</p>\
...
fn parse_oauth_callback(request: &str) -> Option<(String, String)> {
    // Парсим: GET /auth/callback?code=XXX&state=YYY HTTP/1.1
    let url_part = request.lines().next()?;
    ...
    for param in query.split('&') {
        ...
        if kv.len() == 2 {
            match kv[0] {
                "code" => code = url_decode(kv[1]),
                "state" => state = url_decode(kv[1]),
                _ => {}
            }
        }
    }
...
pub async fn exchange_code_for_token(
    ...
    if !ms_response.status().is_success() {
        let error_text = ms_response.text().await.unwrap_or_default();
        return Err(format!("Failed to get token: {}", error_text));
    }
    
    let ms_tokens: MsTokenResponse = ms_response.json()
        .await
